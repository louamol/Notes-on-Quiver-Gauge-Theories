...
\section{Physical setup}
    ...
    \subsubsection*{Mathematical formulation}
        ...
        To summarize in more mathematical terms, our D-branes, together with the stable vector bundle (sheaf) supported thereupon, resolves the transverse Calabi-Yau orbifold which is the vacuum for the gauge theory on the worldvolume as a GIT quotient. \marker{Explain.}
...
\section{Supersymmetric Yang-Mills theories}
    ...
    \subsection{SYM from D-branes}
        ...
        \marker{The end of this section has to be rewritten}
    ...
    \subsection{$\mN=4$ super Yang-Mills theory in $D=4$}\label{sec:N4SCFT}
        ...
        \subsubsection{Superconformal group $\SU(2,2|4)$ and its representations}
            ...
            \begin{itemize}
                ...
                \item \textbf{Poincaré supersymmetries}: they form the 16-dimensional sub group \marker{Why ?} and are generated by $Q^I_\alpha$ and $\bar{Q}^I_{\dalpha}$.
                \item \textbf{Conformal supersymmetries}: they form the 16-dimensional subgroup \marker{Why ?} and are generated by $S_{\alpha I}$ and $\bar{S}^{\dalpha I}$.
            \end{itemize}
...
\section{Algebraic geometry}
    ...
    \subsection{Singularities and resolutions}
        ...
        The \emph{resolution of a singularity} of an algebraic variety $V$ is a non-singular variety $W$ with a proper birational map $W\to V$. For varities over fields of characteristic $0$, it was proven (Hironaka, 1964) that \marker{fill in}.
...
\section{Quivers representations, path algebras, moduli spaces and quiver varieties}
    ...
    \marker{This section has to be rewritten.}
...
\section{The simplest case: smooth transverse space}
    ...
    \subsection{Matter content}
        ...
        \begin{itemize}
            ...
            \end{align}
            Note that this gives $6N^2$ real scalars in total. They are the superpartners of the fermions. \marker{Detail.}
        \end{itemize}
...
\section{Strings on orbifolds}
    ...
    \subsection{Generalities}
        ...
        If $\Gamma$ is a general finite group the condition that $\C^3/\Gamma$ is an Calabi-Yau orbifold means that there must exist a resolution of this orbifold such that the corresponding smooth space is Calabi-Yau, i.e. a crepant resolution. Existence of such a resolution constrains $\Gamma$ \marker{Details ?}, see appendix \ref{sec:CY}.
    ...
    \subsubsection{Further details on the projection}
        ...
        Riemannian symmetric spaces, which are locally isometric to homogeneous spaces $G/H$ have local holonomy isomorphic to $H$. \marker{Continue and detail.}
    ...
    \subsection{Representation theory of the projection}
        ...
        After the projection, the resulting gauge group is given by the $\Gamma$-invariant part of the gauge group, that is $\Hom(\C^N,\C^N)^\Gamma$. We use the superscript $\Gamma$ to indicate that we only keep the trivial representations in the decomposition, that is, we only keep that subspaces that transform trivially. First, we can see that by Schur's lemma \marker{Why ?}
        \begin{equation}
        ...
        \end{equation}
        Now it turns out that in the low energy effective field theory the $\U(1)$ factor of every $\U(N_i)$ decouples \marker{Why ?} so the resulting gauge group is in fact
        \begin{equation}
    ...
    \subsection{A simple example: $S=\C^3/\Z_3$}\label{sec:C3Z3}
        ...
        \subsubsection{Projection}
            ...
            Let us now study the four Weyl fermions $\psi^a$. \marker{Continue.}
...
\section{$\mN=2$ daughter theories}\label{sec:N2QGT}
    ...
    \subsection{$S=\C\times\C^2/\D_n$}
        ...
        \end{equation}
        where each entry $(i,j)$ is an arbitrary block of size $N_i\times N_j$. The explicit derivations are carried out in appendix \ref{app:invconfDn}. It is interesting to notive the main differences with the case of $\Z_n$:
        \begin{itemize}
...
\section{Toric geometry}
    ...
    \subsection{Constructing toric affine varieties}
        ...
        \subsubsection{Toric variety of a cones}
            ...
            \begin{examp*}
                If $\sigma=\Cone(e_1,e_2)$ then we find $U_\sigma=\C^2$. There are three faces: $\rho_1=\Cone(e_1)$, $\rho_2=\Cone(e_2)$ and the origin $0$. These faces can be described by hyperplane $H_{e_2}, H_{e_1}$\marker{Why ?} and $H_{e_1+e_2}$ respectively, so we get
                \begin{align}
                    \C[S_{\rho_1}] &= \C[x,y,y^{-1}],
                    \C[S_{\rho_2}] &= \C[x,x^{-1},y],
                    \C[S_{0}] &= \C[x,y,x^{-1},y^{-1}].
                \end{align}
                They define the varieties $U_{\rho_1}=\C^*\times\C$, $U_{\rho_1}=\C\times\C^*$ and $(\C^*)^2$\marker{Why ?}, which are all open subvarieties of $\C^2$.
            \end{examp*}
        ...
        \subsection{Toric variety of a fan}
            ...
            \begin{equation}
                ...
                \right).
            \end{equation}
            It is clear that the kernel of $\phi$ is $\tilde{G}\equiv\Ker(\phi)=(\C^*)^{n-m}$. \marker{Why ?} One can see that this kernel naturally acts on $\C^n$ as
            \begin{equation}
                \left(
                \begin{array}{ccc}
