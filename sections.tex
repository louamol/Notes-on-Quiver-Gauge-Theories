...
\section{Toric geometry}
    ...
    \subsection{Toric variety of a cone}
    ...
    \subsection{Toric variety of a fan}
        ...
        If $\Delta$ is an $m$-dimensional fan, we denote by $\Delta(j)$ ($j\leq n$) the collection of $j$-dimensional cones in $\Delta$. The set $\Delta(1)\subset\Delta$ is then the collection of all one-dimensional cones in $\Delta$. Each cone in $\Delta(1)$ is generated by a vector of $N=\Z^n$ and we denote those generating vectors by $v_1,\dots,v_n$. Note that we always have $n\geq m$. Each vector $v_i$ has component $v^k_i$ ($i=1,\dots,n$,$k=1,\dots,m$) so we can construct an $m\times n$ matrix by putting the vectors in columns:
        \begin{equation}
            \begin{bmatrix}
            ...
            \end{bmatrix}
        \end{equation}
        The kernel of this map from $\C^n$ to $\C^m$ is $\C^{n-m}$. Let $Q^a$ ($a=1,\dots,n-m$) be vectors that generate this kernel, i.e.
        \begin{equation}
            \sum^n_{i=1}v^k_i Q^a_i=0\label{eq:detQalpha}
        \end{equation}
        for all $a=1,\dots,n-m$ and $k=1,\dots,m$. We can now consider the map
        \begin{equation}
            \phi:\left(
            \begin{array}{ccc}
            ...
            \right).
        \end{equation}
        and it is clear that its kernel is $\tilde{G}\equiv\Ker(\phi)=(\C^*)^{n-m}$\marker. One can see that this kernel naturally acts on $\C^n$ as
        \begin{equation}
            \left(
            \begin{array}{ccc}
        ...
        The last piece of data in the construction isthe zero set $Z_\Delta$. For any subset of $\Delta(1)$ (corresponding to the vectors $v_{i_1},\dots,v_{i_l}$) which does not generate a cone in $\Delta$, we define the algebraic set $V_{i_1,\dots,i_l}=Z(z_{i_1},\dots,z_{i_l})$, i.e. the hyperplane of $\C^n$ defined by $z_{i_1}=\dots=z_{i_l}=0$. $Z_\Delta$ si then defined as the union of all such $V_{i_1,\dots,i_l}$.
        ...
        \begin{prop*}
            A toric variety $\Delta$ is compact if and only its fan $\Delta$ span the $N_\R$.
        \end{prop*}
    ...
    \subsection{Calabi-Yau toric varities}
        ...
        As motivated in the beginning, we are mainly interested in Calabi-Youaffine varities, so Calabi-Yau affine troic varieties in this case. A very convenient property of the toric varieties is that the CY condition is translated into a simple condition on the combinatoric data of the variety.

        Recall that a divisor of an affine variety is a linear combination of codimension-one irreducible subvarieties. A \emph{toric divisor} is a divisor invariant under the action of $G$. Using the homogeneous coordinates $(z_i)$, we can easily construct $G$-invariant subvarieties.

        \begin{prop*}
            The otric variety $X_\Delta$ is CY if and only ifall the vectors $v_i$ in $\Delta$ end on the same hyperplane in $N$, which happens if and only if $\sum^n_{i=1}Q^a_i=0$ for $a=1,\dots,n-m$.
        \end{prop*}
    ...
    \subsection{Example 1: the conifold}
        ...
        \begin{equation}
            \mathcal{C_0}=\frac{\C^3\backslash Z_\Delta}{\C^*}.
        \end{equation}
    ...
    \subsection{Examples 1: the suspended pinch point}
