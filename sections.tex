...
\section{Strings on orbifolds}
    ...
    \subsection{Bi-index notation}
        ...
        \end{equation}
        This notation is very convenient to compute explicitely the invariant configurations, as we will see in several examples. We use the exact same bi-index notations for $X^m$ and $\psi^a$.
...
\section{$\mN=2$ daughter theories}\label{sec:N2QGT}
    ...
    \subsection{$S=\C\times\C^2/\D_n$}
        ...
        \end{equation}
        To find the invariant configurations of the gauge group, we use use the bi-index notation and split the sub-blocks of $A_\mu$ in four categories depending on the dimneisonlity of the representations that they transform in. At last, before goinginto the computation, we mention that it is only necessary to check the invaraince under the two generators of $2\D_n$ to ensure invariance under the whole group.
        \begin{itemize}
        ...
        \begin{equation}
            ...
            \end{bmatrix}.
        \end{equation}
        The partitionning of $X^m$ is exactly the same than $A_\mu$.
        \begin{itemize}
        ...
        Summary of differences when irreps are not all $1$-dimensional:
        \begin{itemize}
            \item gauge field is not necessarily diagonal
            \item scalar fields can be exchanged by R-symmetry
        \end{itemize}
    ...
    \subsection{Constructing toric affine varieties}
        ...
        \subsection{Toric variety of a fan}
            ...
            \begin{equation}
                ...
                \right).
            \end{equation}
            In order to have $\phi(z_1,\dots,z_n)=0$ we must have  It is clear that the kernel of $\phi$ is $\tilde{G}\equiv\Ker(\phi)=(\C^*)^{n-m}$\marker. One can see that this kernel naturally acts on $\C^n$ as
            \begin{equation}
                \left(
                \begin{array}{ccc}
