...
\section{Toric varieties}
    ...
    \subsection{Algebraic approach}
        ...
        An \emph{algebraic group} is a group that is also an algebraic variety and such the product and inversion are regular maps on the variety. Any product of $\C^*$ is an algebraic group that we call \emph{algebraic tori}. A \emph{toric variety} $X$ of dimension $n$ is an algebraic variety containing the algebraic torus $\mathbb{T}=(\C^*)^n$ as a dense open subset, together with a natural action $\mathbb{T}\times X\to X$. An $n$-dimensional algebraic variety is obtained as the quotient
        \begin{equation}
    ...
    \subsection{Fans and cones}
         ...
         A \emph{toic variety} $\boldsymbol{\chi}_\Delta$ is an algebrais variety of dimension $n$ containing an algebraic torus $(\C^*)^n$ as a dense open subset, as such that the torus action on itself extends to an algebraic action on $\chi_\Delta$. For instance, $\C\P^2$ is a toric variety. 
