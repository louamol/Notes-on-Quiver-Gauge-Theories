...
\section{Projection to daughter theories}
    ...
    \subsection{Generalities}
        ...
        If $\Gamma$ is a general finite group the condition that $\C^3/\Gamma$ is an Calabi-Yau orbifold means that there must exist a resolution of this orbifold such that the corresponding smooth space is Calabi-Yau, i.e. a crepant resolution. Existence of such a resolution constrains $\Gamma$ \marker{Details ?}, see section \ref{sec:CY}.


    \subsection{Projection}

        \textcolor{red}{This section is considering D$1$-branes on $\R^6\times\R^4/\Gamma$ extending in the $X^0,X^1$-directions. This is not our usual paradigm.}

        Three types of open string sector states must be considered:
        \begin{itemize}
    ...
    \subsection{Representation theory of the projection}
        ...
        \subsubsection{Invariant configurations}
            
            After the projection, the resulting gauge group is given by the $\Gamma$-invariant part of the gauge group, denoted by $\Hom(\C^N,\C^N)^\Gamma$. The superscript $\Gamma$ is used to indicate that we only keep the trivial representations in the decomposition, that is, we only keep that subspaces that transform trivially. We want to compute $\Hom(\C^N,\C^N)^\Gamma$. First, we can see that by Schur's lemma \marker{Why ?}
            \begin{equation}
            ...
            \begin{result}
                In the daughter theory, the matter fields become a total of $a^{\boldsymbol{4}}_{ij}$ bi-fundamental fermions and $a^{\boldsymbol{6}}_{ij}$ bi-fundamental bosons transforming as the $(\boldsymbol{\textbf{N}_i},\bar{\boldsymbol{\textbf{N}_j}})$ of $\SU(N_i)\times\SU(N_j)$ under the products of gauge groups.
            \end{result}
    ...
    \subsection{Gauge anomaly cancellation}
        ...
        \end{equation}
        where $a_{ij}$ are the elements of the adjacency matrix of the quiver and $n_i$ are the node labels. This condition is equivalent to say that the ranks of tha gauge groups must lie in nullspace of the antisymmetrized adjacency matrix.
    ...
    \subsection{On note on the twisted sector}
...
\section{$\mN=2$ daughter theories}\label{sec:N2QGT}
    ...
    \subsection{$S=\C\times\C^2/\D_n$}
        ...
        \end{equation}
        The projected gauge group is then
        \begin{equation}
            G_{\text{proj}} = \SU(N_0)\times\dots\SU(N_3)\times\SU(2N_4)\times\dots\times\U(2N_{n+3}).
        \end{equation}
        ...
        The anomaly cancellation conditions is
