...
\section{The brane-world paradigm}
    ...
    \subsection{More general context}

        More generally speaking, we can consider a D$p$-brane in type II string theory (type IIA if $p$ is even and type IIB if $p$ is odd). The spacetime is then of the form
        \begin{equation}
...
\section{Projection to daughter theories}
    ...
    \subsection{A simple example: $S=\C^3/\Z_3$}
        ...
        \subsubsection{Quiver}
            ...
            \begin{figure}[H]
                ...
                \end{tikzpicture}
                \caption{Scalar quiver of the $\C^3/\Z_3$ daughter theory.}
                \label{fig:Z3quiver}
            \end{figure}
            ...
            \begin{equation}
                ...
                \end{bmatrix}
            \end{equation}
            which is coherent with the fact that the coefficients of the McKay decompositions of $\boldsymbol{1}\oplus\boldsymbol{1}\oplus\boldsymbol{1}$ are
            \begin{align}
                \begin{split}
        ...
        \subsubsection{Gauge anomaly cancellation}
            ...
            Finally, let us discuss the gauge anomaly cancellation. Our fields transform under the adjoint representation of $\SU(n)$, under the fundamentals of $\SU(N_i)$ and under the anti-fundamentals of $\SU(N_i)$. The adjoint representation being reel, it is self-conjugate and therefore do not contribute to the anomaly. Fundamentals and anti-fundamentals representations of $\SU(N_0)$, $\SU(N_1)$ and $\SU(N_2)$ have opposite contribution. Anomaly cancellation therefore imposes that the number a fields transforming in $\textbf{N}_j$ is equal to the number of fields transforming in $\boldsymbol{\bar{\textbf{N}}}_j$, see appendix \eqref{sec:anomalies}. From the diagram, we can see that there are
            \begin{equation}
                \dim(\SU(N_i)\times\bar{\SU(N_j)}) = (N^2_i-1)(N^2_j-1)
            \end{equation}
            fields transforming in the representation $(\boldsymbol{\textbf{N}}_i,\boldsymbol{\bar{\textbf{N}}}_j)$. Since the gauge group $G_{\text{proj}}$ here possesses three factors, or equivalently since the quiver possesses three vertices, there are three conditions:
            \begin{align}
                \SU(N_0) &: (N^2_1-1)(N^2_0-2) = (N^2_0-1)(N^2_2-1),\\
                \SU(N_1) &: (N^2_2-1)(N^2_1-2) = (N^2_1-1)(N^2_0-1),\\
                \SU(N_2) &: (N^2_0-1)(N^2_2-2) = (N^1_2-1)(N^2_1-1).
            \end{align}
...
\section{$\mN=2$ daughter theories}
    ...
    \subsection{$S=\C\times\C^2/\Z_n$}
        ...
        \begin{figure}[H]
            ...
            \end{tikzpicture}
            \caption{Scalar quiver of the $\C\times\C^2/\Z_3$ daughter theory.}
            \label{fig:2Z3quiver}
        \end{figure}
        ...
        \begin{align}
            &(N^2_i-1)(N^2_{i+1}-1)+(N^2_i-1)(N^2_{i-1}-1)+(N^2_i-1)^2\\
            &=(N^2_{i+1}-1)(N^2_i-1)+(N^2_{i-1}-1)(N^2_i-1)+(N^2_i-1)^2
        \end{align}
