...
\section{Projection to daughter theories}
    ...
    \subsection{$S=\C^3/\Z_n$}
       ...
       where $g$ is the generator of $\Z_n$. The representation $(\rho,V)$ can be decomposed as
        ...
        \begin{align*}
            X_{ij}&\equiv (X_{i\alpha_i,j\beta_j})_{\alpha_i=0,\dots,N_i,\beta_j=0,\dots,N_j}\\
            \bar{X}_{ij}&\equiv (\bar{X}_{i\alpha_i,j\beta_j})_{\alpha_i=0,\dots,N_i,\beta_j=0,\dots,N_j}.
        \end{align*}
        The block $X_{ij}$ is an $N_i\times N_j$ block and transforms under the representation $(\boldsymbol{\textbf{N}_i},\boldsymbol{\bar{\textbf{N}}_j})$ of $\U(N_i)\times\U(N_j)$:
        \begin{align}
            X_{i,i+1} &\in \boldsymbol{\textbf{N}_{i+1}}\otimes\boldsymbol{\bar{\textbf{N}}_i} \cong \Hom(V_{i+1},V_i),\\
            \bar{X}_{i+1,i} &\in \boldsymbol{\textbf{N}_{i}}\otimes\boldsymbol{\bar{\textbf{N}}_{i+1}} \cong \Hom(V_{i},V_{i+1}).
        \end{align}
        So for $n=3$ we have three bi-fundamental scalar fields:
        \begin{equation}
            X_{01}\in(\boldsymbol{\textbf{N}_{1}},\boldsymbol{\bar{\textbf{N}}_0}),\quad
            X_{12}\in(\boldsymbol{\textbf{N}_{2}},\boldsymbol{\bar{\textbf{N}}_1}),\quad
            X_{21}\in(\boldsymbol{\textbf{N}_{2}},\boldsymbol{\bar{\textbf{N}}_0}).
        \end{equation}
        We can draw the quiver of this daughter theory.
        ...
        \begin{equation}
            ...
            \end{bmatrix}
        \end{equation}
        which is coherent with the fact that
        \begin{align}
            \begin{split}
                n_{00} &= 0,\quad n_{01} = 0,\qquad n_{02}=3,\\
                n_{10} &= 0,\quad n_{11} = 3,\qquad n_{12}=0,\\
                n_{20} &= 3,\quad n_{21} = 0,\qquad n_{22}=0.\marker
            \end{split}
        \end{align}
        ...
        Finally, let us discuss the gauge anomaly cancellation. Our fields transform under the adjoint representation of $\SU(n)$, under the fundamentals of $\SU(N_i)$ and under the anti-fundamentals of $\SU(N_i)$. The adjoint representation being reel, it is self-conjugate and therefore do not contribute to the anomaly. Fundamentals and anti-fundamentals representations of $\SU(N_0)$, $\SU(N_1)$ and $\SU(N_2)$ have opposite contribution. imposes that the number a fields transforming in $\textbf{N}_j$ is equal to the number of fields transforming in $\boldsymbol{\bar{\textbf{N}}}_j$, see appendix \eqref{sec:anomalies}. This results in a constrain on the $N_i$'s:
        \begin{equation}
            \marker.
        \end{equation}
