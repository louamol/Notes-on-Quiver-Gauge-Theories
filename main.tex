...
\section{$\mN=2$ daughter theories}
    ...
    \subsection{$S=\C\times\C^2/\Z_n$}
        ...
        We start by considering a representation $(\rho,\C^N)$ of $\Z_n$. A complete set of irreducible representations of $\Z_n$ is given by $\{(\rho_k,V_k)\}_{k=0,\dots,n-1}$ with $V_k=\C$ and
        \begin{equation}
            \rho_i(g)=\zeta^k_n
        \end{equation}
        ...
        For the scalar fields, we need to consider the action of the R-symmetry. The action of $\Z_n$ that we consider leaves the first component of $\C^3$ untouched so we take the action $\boldsymbol{1}\oplus\boldsymbol{2}$ where $\boldsymbol{2}$ is the usual action of $\Z_n$ on $\C^2$. In other words, 
        \begin{equation}
            R(g)=
            \begin{bmatrix}
                1 & 0 & 0 \\
            ...
            \end{bmatrix}.
        \end{equation}
        Or, equivalently, $R(g)\indices{^m_n}=\delta\indices{^m_n}A_n$ with $A_m=(1,1,\zeta_n,\zeta_n,\zeta^{-1}_n,\zeta^{-1}_n)$. According to \ref{eq:transfphi1}-\ref{eq:transfphi2}, the scalar field configurations that are left invariant satisfy
        \begin{equation}
        ...
        \end{align}
        So the scalar fields are split up in three families depending on the way they transform under a gauge transformation.

        Let us draw the quiver for the case $n=3$. We have $2\cdot 9=18$ real scalar fields in $9$ different representations:
        \begin{align}
        ...
        \end{align}
        In every representation there are 2 real scalars, i.e. 1 complex scalars. They are each represented by an arrow between the right representations. The resulting quiver is given \ref{fig:2Z3quiver}.
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}
                \foreach \n in {0,...,2}{
        ...
        \begin{equation}
            ...
            \end{bmatrix}
        \end{equation}
        which is coherent with the fact that the coefficients of the McKay decompositions are
        \begin{align}
            \begin{split}
                n_{00} &= 1,\quad n_{01} = 1,\qquad n_{02}=1,\\
                n_{10} &= 1,\quad n_{11} = 1,\qquad n_{12}=1,\\
                n_{20} &= 1,\quad n_{21} = 1,\qquad n_{22}=1.
            \end{split}
        \end{align}
        It is easy to see how the construction of the the quiver generalizes for arbitrary $n$.

        Finally, let us discuss the gauge anomaly cancellation. Our fields transform under the adjoint representation of $\SU(n)$, under the fundamentals of $\SU(N_i)$ and under the anti-fundamentals of $\SU(N_i)$. The adjoint representation being reel, it is self-conjugate and therefore do not contribute to the anomaly. Fundamentals and anti-fundamentals representations of $\SU(N_0)$, $\SU(N_1)$ and $\SU(N_2)$ have opposite contribution. Anomaly cancellation therefore imposes that the number a fields transforming in $\textbf{N}_j$ is equal to the number of fields transforming in $\boldsymbol{\bar{\textbf{N}}}_j$, see appendix \eqref{sec:anomalies}. In other words, the number of incoming and outgoing arrows must be equal vertex at each vertex. That is, $3N_0=3N_1,3N_1=3N_2$ and $3N_2=3N_0$. It implies that
        %\begin{equation}
        %    N_0=N_1=N_2.
        %\end{equation}
...
\section{$\mN=1$ daughter theories}
    ...
    \subsection{$S=\C^3/\Z_3$}
        ...
        \begin{equation}
            ...
            \end{tabular}.\label{eq:reprrhoZn}
        \end{equation}
        Since $\dim\rho_i=1$, $N_0+N_1+N_2=N$. 
        ...
        \begin{equation}
            ...
            \end{bmatrix}
        \end{equation}
        or, equivalently, on the real scalars as $R(g)=\zeta_3\mathbbm{1}_6$. The scalar field configurations that are left invariant satisfy
        \begin{equation}
            R(g)\indices{^m_n}\rho(g)X^n\rho(g)^{-1}=X^m
        \end{equation}
        for all $g\in\Z_n$. Using the bi-index notations, this becomes
        ...
        \end{equation}
        In every representation ($(\boldsymbol{\textbf{N}_1},\boldsymbol{\bar{\textbf{N}}_0}),(\boldsymbol{\textbf{N}_2},\boldsymbol{\bar{\textbf{N}}_1})$ or $(\boldsymbol{\textbf{N}_2},\boldsymbol{\bar{\textbf{N}}_0})$) there are six real scalars, i.e. 3 complex scalars. They are each represented by an arrow between the right representations, see fig. \ref{fig:Z3quiver}.
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}
                \foreach \n in {0,...,2}{
    ...
    \subsection{$S=\C^3/\Z_n$}
        ...
        Let us now consider the general case of $\Z_n$ acting on $\C^3$. The beginning the the reasoning is very similar to the case $n=3$. We start by considering a representation $(\rho,\C^N)$ of $\Z_n$. A complete set of irreducible representations of $\Z_n$ is given by $\{(\rho_k,V_k)\}_{k=0,\dots,n-1}$ with $V_k=\C$ and
        \begin{equation}
            \rho_i(g)=\zeta^k_n
        \end{equation}
        where $g$ is the generator of $\Z_n$. The representation $(\rho,V)$ can be decomposed as
