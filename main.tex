...
\section{Projection to daughter theories}
    ...
    \subsection{$\Z_n$}
        ...
        We discuss a type II theory with a subsector of D$p$-branes for a given $p$ ($p=0,2,4,6,8$ for type IIA and $p=-1,1,3,5,7,9$ for type IIB).This sector is determined by a choice of representation $(\rho^{(p)},V^{(p)})$ of $\Z_n$. Recall the the irreducible representations of $\Z_n$ are given by $(\rho_k,V_k)$($k=0,\dots,n-1$) with $V_k=\C$ and $\rho_i(g)=\zeta^k_n$ where $g$ is the generator of $\Z_n$. The representation $\rho,V^{(p)}$ can be decomposed as
        \begin{equation}
            (\rho^{(p)},V^{p})=\bigoplus^{n-1}_{i=0}N^{(p)}_i(\rho_i,V_i).
        \end{equation}
        Since $\dim\rho_i=1$, $N^{(p)}\equiv\sum_i N^{(p)}_i = \dim V^{(p)}$. Following what we have found using general representation theory, the gauge group $\U(N^{(p)})$ is now broken to
        \begin{equation}
            G_{\text{proj}}=\bigotimes^{n-1}_{i=0}~\U(N^{(p)}_i).
        \end{equation}
        
        
        For the gauge field, we then need a bi-index notation: $A_{i\alpha_i;j\beta_j}$ with $i,j=A,\dots,n-1$ and $\alpha_i,\beta_i=1,\dots,N^{()p}_i$. We sometimes simply write $A_{i\alpha;j\beta}$. The gauge field transform on the adjoint representation of $G_{\text{proj}}$:
        \begin{equation}
            A_{i\alpha;i\beta}\to \rho_i(g)A_{i\alpha;j\beta}\rho_j(g)^{-1}=\zeta^i_nA_{i\alpha;j\beta}\zeta^{-j}_n=\zeta^{i-j}_nA_{i\alpha;j\beta}
        \end{equation}
        so we clearly see that only the components with $i=j$ are left invariant and that the gauge field has a block diagonal form. The block $i=j$ is an $N^{(p)}_i\times N^{(p)}_i$ block and transforms under $(\rho_i,V_i)^{N^{(p)}_i}$.
