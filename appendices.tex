\section{Supersymmetric Yang-Mills theory from D-branes}
    ...
    The end of this section has to be rewritten \marker.
...
\section{Elements of graph theory}
    ...
    \subsection{Quiver diagrams}
        ...
        A \emph{quiver} $Q=(E,V,t,h)$ is a piece of combinatorial data where $E$ is a set of of edges, $V$ is a set of vertices, and $h,t:V\to E$ are two maps that respectively assign a head and a tail to every arrow. A \emph{morphism} $m=(m_V,m_E)$ between two quivers $(V,E,t,h)$ and $(V',E',t',h')$ consists in two maps
        \begin{align}
            m_V &: V\to V'\\
            m_E &: E\to E'
        \end{align}
        such that $m_V\circ t = t'\circ m_E$ and $m_V\circ h = h'\circ m_E$.

        The \emph{line graph} $L(G)$ of a graph $G$ is the graph such that
        \begin{itemize}
            \item each vertex of $L(G)$ represent a line of $G$
            \item two vertices of $L(G)$ are adjacent if and only if their corresponding edges share a common endpoint in $G$.
        \end{itemize}
        In other words, it is the intersection graph of the edges of $G$, where each edge by the set of its two endpoints. Whitney graph isomorphism theorem states that two graphs are isomorphic if and only if their line graphs are isomorphic, with only one exception: $K_3$ and $K_{1,3}$.
