\section{Supersymmetric Yang-Mills theory from D-branes}
...
\section{Quivers representations, path algebras, moduli spaces and quiver varieties}
    ...
    \subsection{Quiver representations}
        ...
        The whole point to all this is the following. There is an important link to make between the representation of a quiver $Q$ and a representation of its path algebra $kQ$: they are equivalent. First, recall that a representation of a $k$-algebra $A=(K,R)$ on a $k$-vector space $V$ is a $k$-linear morphism $A\to\End(M)$. Now it is not to difficult to see that giving a $k$-linear morphism $A\to\End(M)$ is equivalent to giving a structure of $R$-module to $V$.
        
        $(k,R)$ (ring $R$ provided with the action of a field $k$) is the choice of a $R$-module (abelian group provided with the action of a ring $R$). But the $k$-vector space associated to each vertex of $Q$ by a representation are in particular \marker. Gabriel's theorem could then be equivalently phrased as: the path algebra of any quiver has fnite representations if and only if it is ADE.
        
        This can be rephrased using a more categorical language. It is now clear that the finite-dimensional representations of a quiver with relations form a category, which we denote by $\texttt{fRep}(kQ,R)$. If $(Q,I(R))$ is a quiver with relations and $A=kQ/I$ its path algebra, then the set of finite-dimensional module of $A$ is also a category that we denote by $\texttt{fdmod}A$.
        \begin{lemma*}
